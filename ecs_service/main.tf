...
resource "aws_ecs_service" "nginx_service" {
  ...
  load_balancer {
    target_group_arn = aws_lb_target_group.alb-target-group-nginx-80.arn
...
resource "aws_lb_target_group" "alb-target-group-nginx-80" {
  name     = "alb-target-group-nginx-80"
  port     = 80
  protocol = "HTTP"
